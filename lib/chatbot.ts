...
export const TOPICS: ChatTopic[] = [
  ...
  },
  {
    ...
    answer:
      'Ten pieces of work are written up on this site — recent builds first, then the roles behind them:\n\n• Load Away — a satirical iOS game whose loading bar only fills when you look away (2026)\n• PopShot!! — an iOS app that turns group travel photos into a game (2026)\n• Hisplora — location-based heritage quests you walk, checkpoint by checkpoint (2026)\n• ReguLens — autonomous regulatory monitoring for small exporters (2026)\n• Shopify Automation — an agent pipeline turning supplier links into review-ready drafts (2026)\n• Briefly — a workspace where the agent works on the same surface as the human (2026)\n• Logic & Code — a server-scored try-out platform at gerbangdev.com (2026)\n• ABSATA — a Flutter attendance app for staff at DPR RI (2024)\n• Higgz Academia — freelance edtech problem-solving and video tutorials (2023–2024)\n• MDP Lecturing — assistant lecturer work at Multi Data Palembang (2023–2024)\n\nThe works index also lists earlier coursework and client work — Balive, PELICAN, COLAB, Suwotify, Filmu — without full write-ups. Ask me about any of them.',
    links: [{ label: 'Browse all works', href: '/works' }],
    next: ['loadaway', 'regulens', 'logicandcode'],
  },
  {
    id: 'loadaway',
    chip: 'Load Away',
    ...
    answer:
      "Load Away is a satirical iOS game built at the Apple Developer Academy: the loading bar advances only while you are NOT looking at it. Umar was one of three engineers on a five-person team, and built the ARKit gaze pipeline — head angle re-derived as face-to-camera geometry, five anti-cheat guards on top (eye-gaze peek check, eyes-closed path, frozen-mesh detector, identity lock, peek tax) — plus the watchOS companion streaming live heart rate into the bar's volatility.",
    links: [
  ...
  },
  {
    ...
    answer:
      'PopShot!! turns documenting a group trip into the game itself: everyone gets photo challenges, each person picks an exclusive sidequest, and the shared album unlocks only once the trip ends. Umar built the iOS side in Swift for a six-person team — auth, the adaptive landing, join-by-code, the real-time Waiting Room over WebSocket, the camera and its challenge overlay, and the Memories and album screens. The Go backend was built by teammates.',
    links: [
      { label: 'Read the case study', href: '/works/popshot' },
      { label: 'Try it on TestFlight', href: 'https://testflight.apple.com/join/dts9hnCS' },
    ],
    next: ['loadaway', 'hisplora', 'skills'],
  },
  {
    id: 'shopifyauto',
  ...
  },
  {
    ...
    answer:
      'Hisplora turns a heritage visit into a walking quest: linear checkpoints, a piece of lore unlocked at each one, and a shareable recap at the end. Umar wrote the product overview and the quest lore — including "Jejak Terakhir Badung", five checkpoints across roughly 2.2 km of Denpasar — and built the iOS side with a five-person team. The mechanics split by the character of each place: quiet at a puri or pura, interactive in markets and squares.',
    links: [
      { label: 'Read the case study', href: '/works/hisplora' },
      { label: 'Source on GitHub', href: 'https://github.com/imeldamayanti/challenge5' },
    ],
    next: ['popshot', 'loadaway', 'academy'],
  },
  {
    id: 'regulens',
    chip: 'ReguLens',
    ...
    answer:
      'ReguLens is autonomous regulatory monitoring for small food and beverage exporters — not a regulation chatbot. It keeps a structured compliance twin of a product and reconciles it against incoming clauses, so a changed rule flips the verdict on its own, names the regulation, and quotes the clause. Umar worked both sides of the stack as one of two contributors (39 of 83 commits): the impact engine, the guardrail, query retrieval, streaming extraction, and the operations console. 618 tests run green from a clean checkout with no GCP account.',
    links: [
      { label: 'Read the case study', href: '/works/regulens' },
    ...
    next: ['shopifyauto', 'briefly', 'skills'],
  },
  {
    id: 'briefly',
    chip: 'Briefly',
    ...
    answer:
      'Briefly is a content workspace where the agent works on the same surface as the human: it reads the view the person has open through WebMCP, and the finished brief lands in the application library — with a status and links to its trend and product — instead of in a chat transcript. Umar built the trend table and detail drawer with ten of the registered tools, the calendar and performance views with the last two, and the stores behind them, over a two-day hackathon with three people.',
    links: [
      { label: 'Read the case study', href: '/works/briefly' },
      { label: 'Open the demo', href: 'https://briefly-1.vercel.app' },
    ],
    next: ['regulens', 'shopifyauto', 'skills'],
  },
  {
    id: 'logicandcode',
    chip: 'Logic & Code',
    ...
    answer:
      'Logic & Code (gerbangdev.com) is an independent try-out platform — not an official Apple product — for people preparing for the Apple Developer Academy Indonesia selection. Umar built it solo. The rule that shapes it: anything that decides an outcome lives on the server. The client never reads the questions table at all, scoring runs inside the database, the attempt deadline is written server-side, and tokens are credited only by an HMAC-verified webhook. The free balance rolls back up to 50 every 72 hours, computed lazily with no cron job.',
    links: [
      { label: 'Read the case study', href: '/works/logic-and-code' },
      { label: 'Open gerbangdev.com', href: 'https://gerbangdev.com' },
    ],
    next: ['regulens', 'skills', 'hire'],
  },
  {
    id: 'absata',
    chip: 'ABSATA',
